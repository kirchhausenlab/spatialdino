...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="run_stage_2",
        action="store_true",
        help="Run Stage 2 classification.",
    )
    parser.add_argument(
        "--skip-stage-2",
        dest="run_stage_2",
        action="store_false",
        help="Skip Stage 2 classification.",
    )
    ...
    parser.add_argument(
        "--seg-tif",
        default=None,
        help="Training segmentation TIFF used for density estimation.",
    )
    parser.add_argument(
        "--valid-mask-tif",
        default=None,
    ...
    parser.add_argument(
        "--device", default=None, help="'cuda' or 'cpu' (auto if omitted)."
    )
    parser.add_argument(
        "--feature-batch",
        type=int,
        default=32,
        help="Feature batch size for Stage 2 classification.",
    )
    parser.add_argument(
        "--kde-points", type=int, default=512, help="Density grid size."
    )
    parser.add_argument(
        "--kde-max-samples",
        type=int,
        default=200000,
        help="Max samples per class in Stage 1.",
    )
    parser.add_argument(
        "--kde-bandwidth",
        type=float,
        default=None,
        help="Manual KDE bandwidth; omit for auto.",
    )
    parser.add_argument(
        "--density-method",
    ...
    parser.add_argument(
        "--seed", type=int, default=1337, help="Random seed for Stage 1 sampling."
    )
...
def load_valid_mask(
    ...
    with tifffile.TiffFile(valid_mask_tif) as tif:
        if not tif.series:
            raise ValueError(
                f"{valid_mask_tif} does not contain a readable TIFF volume."
            )
    ...
    if valid_mask.ndim == 2:
        if valid_mask.shape != (y_size, x_size):
            raise ValueError(
                f"Valid mask shape {valid_mask.shape} != {(y_size, x_size)}."
            )
...
def reorder_lists_to_current(
    ...
    if len(set(current_names)) != len(current_names) or len(set(saved_names)) != len(
        saved_names
    ):
        if list(current_names) != list(saved_names):
            raise RuntimeError(
                "Cannot align cached densities to current feature order."
            )
    ...
    for values in lists:
        reordered.append([
            np.asarray(values[index], dtype=np.float32) for index in index_order
        ])
    return reordered
...
class UpsampledFeatureSampler:
    ...
    def _build_xy_grid(self) -> Tensor:
        ...
        y_coords = (
            (torch.arange(y_size, device=self.device, dtype=torch.float32) + 0.5)
            * 2.0
            / y_size
        ) - 1.0
    ...
    def _grid_for_z_indices(self, z_indices: Sequence[int]) -> Tensor:
        ...
        z_coords = (
            (torch.tensor(key, device=self.device, dtype=torch.float32) + 0.5)
            * 2.0
            / z_size
        ) - 1.0
    ...
    def _get_feature_range_tensor(self, start: int, end: int) -> Tensor:
        ...
        lr_chunk = np.moveaxis(
            np.asarray(self.lr_feats[..., start:end], dtype=np.float32), -1, 0
        )
        input_tensor = (
            torch
            .from_numpy(ensure_contiguous(lr_chunk))
            .unsqueeze(0)
...
def collect_samples_from_timepoint(
    ...
    *,
    ...
    if seg.shape != raw_shape:
        raise ValueError(
            f"Training segmentation shape {seg.shape} does not match raw shape {raw_shape}."
        )

    valid_mask = (
        load_valid_mask(valid_mask_tif, shape_zyx=raw_shape)
    ...
    for start in range(0, feature_total, batch_features):
        ...
        for feature_index in range(start, end):
            ...
            x_grid = torch.linspace(
                value_min, value_max, bin_count, device=device, dtype=torch.float32
            )

            def histogram_1d(values: np.ndarray) -> torch.Tensor:
                if values.size == 0:
                    return torch.zeros(bin_count, device=device, dtype=torch.float32)
                tensor = torch.from_numpy(np.asarray(values, dtype=np.float32)).to(
                    device=device, dtype=torch.float32
                )
                indices = torch.clamp(
                    (
                        (tensor - value_min)
                        / (value_max - value_min + 1e-20)
                        * (bin_count - 1)
                    ).long(),
                    0,
                    bin_count - 1,
            ...
            s_bg = F.conv1d(h_bg.view(1, 1, bin_count), kernel, padding=pad).view(
                bin_count
            )
            s_fg = F.conv1d(h_fg.view(1, 1, bin_count), kernel, padding=pad).view(
                bin_count
            )
...
def classify_timepoint(
    ...
    *,
    ...
    for z_indices in iter_z_index_chunks(z_size):
        ...
        cnt_bg = torch.zeros(
            (chunk_depth, y_size, x_size), device=device, dtype=torch.int32
        )
        cnt_fg = torch.zeros(
            (chunk_depth, y_size, x_size), device=device, dtype=torch.int32
        )
        ...
        for start in range(0, sampler.channel_count, feature_batch):
            ...
            denom = p_bg + p_fg
            zero_mask = denom == 0
            prob_bg = torch.where(zero_mask, 0.5, p_bg / denom).view(
                end - start, chunk_depth, y_size, x_size
            )
            prob_fg = torch.where(zero_mask, 0.5, p_fg / denom).view(
                end - start, chunk_depth, y_size, x_size
            )
...
def load_or_compute_densities(
    ...
    *,
    ...
    if first_lr.ndim != 4:
        raise ValueError(
            f"{first_timepoint.lr_path} must be a 4D array with shape [Z, Y, X, C]."
        )
    ...
    if not params.run_density_estimation:
        ...
        x1_all, f1_all, x2_all, f2_all, saved_names = load_densities_npz(
            params.densities_path
        )
        reordered = reorder_lists_to_current(
            current_feature_names, saved_names, [x1_all, f1_all, x2_all, f2_all]
        )
    ...
    selected_timepoint = next(
        (item for item in timepoints if item.name == params.training_timepoint), None
    )
    if selected_timepoint is None:
        raise ValueError(
            f"Training timepoint {params.training_timepoint!r} was not found in the input folder."
        )
    ...
    save_densities_npz(
        params.densities_path, x1_all, f1_all, x2_all, f2_all, feature_names
    )
...
def run_probability_map(input_path: Path, *, params: ProbabilityMapParams) -> Path:
    ...
    output_path = (
        params.output_path.expanduser().resolve()
    ...
    x1_all, f1_all, x2_all, f2_all = load_or_compute_densities(
        timepoints, params, device=device
    )
    ...
    for index, timepoint in enumerate(timepoints, start=1):
        print(
            f"[probmap] Processing {timepoint.name} ({index}/{len(timepoints)})",
            flush=True,
        )
        classify_timepoint(
            timepoint,
...
def main() -> None:
    ...
    valid_mask_tif = (
        Path(args.valid_mask_tif).expanduser().resolve()
