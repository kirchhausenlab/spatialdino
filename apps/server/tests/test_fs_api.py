...
class FsApiTests(unittest.TestCase):
    ...
    def test_list_returns_directories_under_allowed_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.dict(
                ...
                payload = fs_api.fs_list(
                    path=str(root), page="1", pageSize="50", sort="name", order="asc"
                )

            self.assertEqual(payload["path"], str(root))
            self.assertEqual(
                [item["name"] for item in payload["items"]], ["alpha", "beta"]
            )
    ...
    def test_mkdir_creates_directory_and_invalidates_cached_parent_listing(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            payload = fs_api.CreateDirRequest.model_validate({
                "parentPath": str(root),
                "name": "gamma",
            })
            ...
            with patch.dict(
                ...
                before = fs_api.fs_list(
                    path=str(root), page="1", pageSize="50", sort="name", order="asc"
                )
                created = fs_api.fs_mkdir(payload)
                after = fs_api.fs_list(
                    path=str(root), page="1", pageSize="50", sort="name", order="asc"
                )
            ...
            self.assertEqual(
                [item["name"] for item in after["items"]], ["alpha", "gamma"]
            )
    ...
    def test_mkdir_rejects_duplicate_name(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                context.exception.detail, "An entry with that name already exists."
            )
